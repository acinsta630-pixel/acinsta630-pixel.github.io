...
window.addEventListener("load", () => {
  ...
  if (user) {
    ...
    // اگر صفحه لاگین وجود نداشت (مستقیما تو chat.html بودیم)
    loadChats();
  }
});
...
// ===============================
// LOAD & SAVE CHATS (PERSISTENT FIX)
// ===============================

function loadChats() {
  const saved = localStorage.getItem("amirAI_chats");
  const savedActive = localStorage.getItem("amirAI_activeChat");
  ...
  if (!Array.isArray(chats) || chats.length === 0) {
    ...
    // بررسی آی‌دی آخرین چت فعال از حافظه
    const found = chats.find(c => c.id == savedActive);
    if (found) {
      activeChat = found.id;
    } else {
...
function saveChats() {
  ...
  if (activeChat) {
    localStorage.setItem("amirAI_activeChat", activeChat);
  }
}
...
// ===============================
// SEND MESSAGE (FAILSAFE FOR MOBILE)
// ===============================
...
async function sendMessage() {
  ...
  // بازیابی فوق‌سریع چت در صورت خالی بودن activeChat رو موبایل
