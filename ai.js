...
// ===============================
// LOAD & SAVE CHATS (PERSISTENT AFTER REFRESH)
// ===============================
...
function loadChats() {
  const savedChats = localStorage.getItem("amirAI_chats");
  const savedActiveId = localStorage.getItem("amirAI_activeChat");
  ...
  if (!Array.isArray(chats) || chats.length === 0) {
    ...
    // بازگردانی چت فعلی یا انتخاب چت اول
    if (savedActiveId && chats.some(c => c.id == savedActiveId)) {
      activeChat = Number(savedActiveId);
    } else {
...
function createChat() {
  const chat = {
    ...
    title: "گفتگوی جدید",
    messages: [
      { text: "سلام! من AI امیر هستم. چطور می‌تونم کمکت کنم؟", type: "bot" }
    ],
